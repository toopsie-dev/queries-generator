...
const error      = ref('')
const queryMode  = ref<'fetch' | 'convert'>('convert')
...
const POSITIONAL_COLS: (keyof RowData)[] = ['account', 'serial', 'partno', 'workorder', 'revision']
...
function processFile(file: File) {
  ...
  reader.onload = (e) => {
    try {
      ...
      const matchedName = wb.SheetNames.find(n => { const k = n.toLowerCase().replace(/[\s_\-]/g, ''); return k.includes('conversion') || k.includes('convert') })
      ...
      rows.value = dataRows
        .map((r) => {
          const row: RowData = { account: '', serial: '', partno: '', workorder: '', revision: '' }
...
// ── Group by identical field-change signature ──────────────────────────────────
...
const groups = computed<ConversionGroup[]>(() => {
  if (hasDuplicates.value) return []
  const map = new Map<string, ConversionGroup>()
  const order: string[] = []
  for (const r of rows.value) {
    const key = `${r.account}||${r.partno}||${r.workorder}||${r.revision}`
    if (!map.has(key)) {
      map.set(key, { account: r.account, partno: r.partno, workorder: r.workorder, revision: r.revision, serials: [] })
      order.push(key)
    }
...
function generateGroup(g: ConversionGroup): string {
  const fields: string[] = []
  ...
  const whereBase  = `account = ${q(g.account)}`

  if (g.serials.length === 1) {
    return `UPDATE card SET ${setClause} WHERE ${whereBase} AND serialno = ${q(g.serials[0])}`
  }
  const serialList = g.serials.map((s) => q(s)).join(',\n')
  return `UPDATE card SET ${setClause} WHERE ${whereBase} AND serialno IN (\n${serialList}\n)`
}

const plainText = computed(() => {
  if (hasDuplicates.value) return ''
  return groups.value.map(generateGroup).filter(Boolean).join('\n\n')
})

// ── Fetch query (SELECT all serials grouped by account) ────────────────────────
  ...
  return line
    ...
    .map((part) => {
      ...
      return escapeHtml(part)
        ...
        .replace(/\b(partno|workorder|revision)\b/g, '<span class="sql-proc">$1</span>')
    })
    .join('')
}
...
const highlightedSql = computed(() => {
  ...
  return plainText.value.split('\n')
    .map((line) => highlightLine(line, 'UPDATE|SET|WHERE|and|in'))
    .join('\n')
})
...
const missingCols = computed(() => {
  ...
  return (['account', 'serial'] as (keyof RowData)[]).filter((k) => r[k] === '')
})
</script>
  ...
  <div class="p-8 space-y-6 max-w-full">
    ...
    <div class="border-b border-border pb-5">
      ...
      <p class="text-sm text-muted-foreground mt-1">
        ...
        <code class="text-[#569cd6] font-mono text-xs">UPDATE card SET</code>
        queries. Only non-empty fields are included. Rows with identical changes are grouped.
      </p>
    </div>
    ...
    <div class="rounded-md border border-border bg-muted/20 px-4 py-3 text-xs text-muted-foreground font-mono flex flex-wrap gap-2 items-center">
      ...
      <span
        v-for="col in ['Account', 'Serial', 'Model / partno', 'Workorder', 'Revision']"
        :key="col"
    ...
    <div v-if="rows.length && !hasDuplicates" class="space-y-3">
      ...
      <div class="flex items-center gap-2">
        ...
        >Fetch</Button>
        <Button
          :variant="queryMode === 'convert' ? 'default' : 'outline'"
          size="sm"
          @click="queryMode = 'convert'; copied = false"
        >Convert</Button>
      </div>
      ...
      <div class="overflow-auto rounded border border-border max-h-52">
        <table class="w-full text-xs font-mono border-collapse">
          <thead class="sticky top-0 bg-zinc-900 z-10">
            <tr>
              <th v-for="col in ['account','serial','model / partno','workorder','revision']" :key="col"
          ...
          <tbody>
            ...
            >
              ...
              <td class="px-3 py-1.5 whitespace-nowrap text-muted-foreground">{{ row.partno || '—' }}</td>
